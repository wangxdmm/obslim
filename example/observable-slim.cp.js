  ...
  var _create = function (target, domDelay, originalObservable, originalPath) {
    ...
    if (target instanceof Array) {
      if (!target.hasOwnProperty("__slim_length"))
        Object.defineProperty(target, "__slim_length", { enumerable: false, value: target.length, writable: true });
    ...
    var _notifyObservers = function (numChanges, immediate = false) {
    ...
    var handler = {
      get: function (target, property) {
        ...
        if (target instanceof Array && ["push", "pop", "unshift", "shift", "splice"].indexOf(property) > -1) {
          ...
          if (
            !(["pop", "shift"].indexOf(property) > -1 && target.length === 0) &&
            ...
            Promise.resolve().then(() => {
              ...
              changes.push({
                operation: targetBuffer.operate,
                operationType: "unitedArrayChange",
                target: target,
                property: undefined,
                newValue: target,
                oldValue: targetBuffer.value,
        ...
        if (property === "__slim_getTarget") {
          return target;
        } else if (property === "__slim_isProxy") {
          return true;
          // from the perspective of a given observable on a parent object, return the parent object of the given nested object
        } else if (property === "__slim_getParent") {
          return function (i) {
            if (typeof i === "undefined") var i = 1;
            var parentPath = _getPath(target, "__slim_getParent").split(".");
            parentPath.splice(-(i + 1), i + 1);
            return _getProperty(observable.parentProxy, parentPath.join("."));
          };
          // return the full path of the current object relative to the parent observable
        } else if (property === "__slim_getPath") {
          // strip off the 17 characters for ".__slim_getParent"
          var parentPath = _getPath(target, "__slim_getParent");
      ...
      deleteProperty: function (target, property) {
        ...
        if (!(target instanceof Array) || !target.__slim_arrOpPending) {
          changes.push({
            type: "delete",
            target: target,
            property: property,
            ...
            currentPath: _getPath(target, property).split("."),
            proxy: proxy,
            index: ++index,
          });
        }
      ...
      set: function (target, property, value, receiver) {
        ...
        if (
          targetProp !== value ||
          originalChange === false ||
          (property === "length" && target instanceof Array && target.__slim_length !== value)
        ) {
          ...
          // so in order to accurately report the correct previousValue for the .length, we have to use a helper property.
          if (property === "length" && target instanceof Array && target.__slim_length !== value) {
    ...
    var __slim_targetPosition = target.__slim_targetPosition;
  ...
  return {
    ...
    create: function (target, domDelay, observer) {
      ...
      if (target.__slim_isProxy === true) {
        var target = target.__slim_getTarget;
